...
export class LcaSidebarComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    const userDetailsSub = this.currentUser$.subscribe(user => {
      this.currentUserDetails = user; // [Keep a local copy for easier checks]
    ...
    const routerEventsSub = this.router.events.pipe(
      ...
      distinctUntilChanged() // [Only react to actual URL changes]
      ...
      // Exception for specific registration modal triggers - this logic might be too complex.
      // It's generally better to keep sidebar interactions simple or manage modal state via a service.
      // For now, we'll rely on the user being on the home page to change account type.
    });
