...
import { LawyerRegisterParametersDto } from '../../../../core/models/lawyer.models';
import { finalize } from 'rxjs/operators';

@Component({
  selector: 'app-lawyer-register-modal',
  templateUrl: './lawyer-register-modal.component.html',
  styleUrls: ['./lawyer-register-modal.component.css']
})
export class LawyerRegisterModalComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.registerForm = this.fb.group({
      // The API for /api/lawyer/register/account expects 'phone' and 'address'.
  ...
  onSubmit(): void {
    ...
    if (!currentUserId) {
        this.toastService.showError("Cannot register lawyer account: User not identified.");
        return;
    }

    this.isLoading = true;
    const payload: LawyerRegisterParametersDto = {
    ...
    this.lawyerService.registerAccount(payload).pipe(
